...
public class NetworkHandler : NetworkBehaviour
{
    ...
    void OnClientConnected(ulong clientId)
    {
        if (!IsHost) {
            ...
            if (clients.Contains(clientId))
                throw new System.Exception($"Network Error : two clients with the same id {clientId}.");
    ...
    void OnClientDisconnected(ulong clientId)
    {
        if (!IsHost) {
            ...
            if (!clients.Contains(clientId))
                throw new System.Exception($"Network Error : tried to disconnect client with id {clientId} but could not find it.");
